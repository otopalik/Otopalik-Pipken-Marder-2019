...
mydir = dir('Output/Summation Simulations Dropoff vs Taper 2 to 0.4 microns')
...
dir_bias_peaks_percent = (cp_peak./cf_peak - 1)*100;
dir_bias_peaks_sub = cp_peak - cf_peak;
dir_bias_int = cp_int./cf_int;
dir_bias_int_sub = cp_int-cf_int;
linearity_int = cp_int-ind_int;
linearity_proportion_int = cp_int./ind_int;
...
subplot(4,1,1)
for i = ind_5000
    plot(Ra_short_vect, dir_bias_int(i,:), '.-', 'MarkerSize', 10, 'LineWidth', 1); hold on
    ylim([-8 8]); ylabel('Directional Bias'); xlabel('Ra (Ohms*cm)'); 
    xlim([50 300]); title('Rm = 5000')
    box off
end
...
for i = ind_20000
    plot(Ra_short_vect, dir_bias_int(i,:), '.-', 'MarkerSize', 10, 'LineWidth', 1); hold on
    ylim([-8 8]); ylabel('Directional Bias'); xlabel('Ra (Ohms*cm)'); 
    xlim([50 300]); title('Rm = 20000')
    box off
end
legend('Taper', 'Drop @ 0.1', 'Drop @ 0.2', 'Drop @ 0.3', 'Drop @ 0.4', 'Drop @ 0.45', 'Drop @ 0.5')

for i = 1:4
    subplot(4,1,i)
    plot([50 300], [0 0], 'k-', 'LineWidth', 1); ylim([-0.2 0.2])
end
